function search(){
    ...
    var itemsCategory = document.getElementsByClassName("itemCategory");
    var categoryLength = document.getElementsByClassName("itemCategory").length;
    ...
    for(i = 0; i < categoryLength; i++){
        filter = itemsCategory[i].innerHTML.toLowerCase();
        console.log(filter);
        if(filter.match(input)){
            document.getElementsByClassName("item")[i].style.display="block";
        }
...
function searchTable(){
    var i,filter,input;
    input = document.getElementById("tablesearch").value.toLowerCase();

    var items = document.getElementsByClassName("table");
    var length = document.getElementsByClassName("table").length;
...
function displayItems(tableId,item,price){
  ...
  var text ="";
  text+='<table><tr><th>Item</th><th>Price</th><th>Quantity</th></tr>';
...
localStorage.setItem("tomatosoup",100);
localStorage.setItem("manchowsoup",120);
localStorage.setItem("muttonkebab",160);
localStorage.setItem("sheekkebab",140);
localStorage.setItem("chickentikka",165);
localStorage.setItem("eggfriedrice",170);
localStorage.setItem("chickenfriedrice",190);
localStorage.setItem("rabdi",80);
localStorage.setItem("gulabjamun",60);
